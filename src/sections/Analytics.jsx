...
const Analytics = () => {
  ...
  return (
    <div className="fade-in" style={{ paddingBottom: '4rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      {/* 1. Analytics Header - Mirrored from Dashboard */}
      <div className="card shadow-premium" style={{ 
        ...
        <div style={{ position: 'relative', zIndex: 10 }}>
          <p style={{ fontSize: '0.9rem', color: '#db2777', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 800, marginBottom: '0.5rem' }}>Comprehensive Performance Analytics</p>
          ...
          <p style={{ color: '#475569', fontSize: '1.1rem', maxWidth: '500px', lineHeight: 1.6, margin: 0 }}>
            Deep dive into your LSRW proficiency metrics and historical growth data.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '2rem', position: 'relative', zIndex: 10, background: 'rgba(255,255,255,0.7)', padding: '1.5rem 2.5rem', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.8)', backdropFilter: 'blur(10px)', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <span style={{ fontSize: '0.8rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 800 }}>Global Rank</span>
            <span style={{ fontSize: '1.25rem', fontWeight: 800, color: '#10b981' }}>Top 5%</span>
          </div>
          <div style={{ width: '1px', background: 'rgba(0,0,0,0.1)' }}></div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <span style={{ fontSize: '0.8rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 800 }}>Avg Complexity</span>
            <span style={{ fontSize: '1.5rem', fontWeight: 800, color: '#0f172a' }}>8.4/10</span>
          </div>
        </div>
      </div>
            ...
            <div style={{ height: '300px', width: '100%', marginTop: '1rem' }}>
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 12, fontWeight: 700 }} domain={[0, 100]} />
                  <Tooltip contentStyle={{ borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)', fontSize: '0.9rem', fontWeight: 700, padding: '12px' }} cursor={{ fill: '#f8fafc' }} />
                  <Legend verticalAlign="bottom" height={36} iconType="circle" formatter={(value) => <span style={{ color: '#475569', fontSize: '0.85rem', fontWeight: 700 }}>{value}</span>} />
                  <Bar dataKey="lsrw" name="LSRW Average" barSize={24} fill="#3b82f6" radius={[6, 6, 0, 0]} animationDuration={1500} />
                  <Bar dataKey="ai" name="AI Confidence" barSize={24} fill="#8b5cf6" radius={[6, 6, 0, 0]} animationDuration={1800} />
                  <Line type="monotone" dataKey="trend" name="Growth Trend" stroke="#10b981" strokeWidth={4} dot={{ r: 6, fill: '#10b981', strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 8, strokeWidth: 0 }} animationDuration={2000} />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="card shadow-sm" style={{ padding: '2rem', background: '#fff', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0f172a', margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <Award size={20} color="#10b981" /> Performance Badges
              </h3>
            </div>
      ...
      <div className="card shadow-sm" style={{ padding: '2rem', background: '#fff', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#0f172a', margin: 0, display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <Clock size={20} color="#64748b" /> Historical Data
          </h3>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '800px' }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                <th style={{ padding: '1rem', textAlign: 'left', fontSize: '0.85rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Assessment</th>
                <th style={{ padding: '1rem', textAlign: 'left', fontSize: '0.85rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Date</th>
                <th style={{ padding: '1rem', textAlign: 'center', fontSize: '0.85rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Score</th>
                <th style={{ padding: '1rem', textAlign: 'left', fontSize: '0.85rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Level</th>
              </tr>
            </thead>
            <tbody>
              {recentActivity.map((test, index) => (
                <tr key={index} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ padding: '1.25rem 1rem' }}>
                    <span style={{ padding: '4px 12px', background: test.statusBg, color: test.statusColor, borderRadius: '20px', fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase' }}>{test.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
